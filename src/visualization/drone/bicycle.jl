function build_drone!(vis::Visualizer, model::BicycleGame; name::Symbol=:Robot, r_col=0.08, r_cost=0.5, α=1.0, r=0.15)
	r = convert(Float32, r)
    p = model.p
...
function set_drone!(vis::Visualizer, model::BicycleGame, s::AbstractVector; name::Symbol=:Robot)
    p = model.p
    pz = model.pz
	d = Algames.dim(model)
    for i = 1:p
