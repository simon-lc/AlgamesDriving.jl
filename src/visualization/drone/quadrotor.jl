function build_drone!(vis::Visualizer, model::QuadrotorGame; name::Symbol=:Robot, r_col=0.08, r_cost=0.5, α=1.0, r=0.15)
	r = convert(Float32, r)
    p = model.p
...
function set_drone!(vis::Visualizer, model::QuadrotorGame, s::AbstractVector; name::Symbol=:Robot)
    p = model.p
    pz = model.pz
    for i = 1:p
		x = s[pz[i][1:3]]
