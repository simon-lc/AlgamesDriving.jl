...
# We use a LQR cost
Q = [Diagonal(3*SVector{model.ni[1],T}([1.0, 0.0, 1.0, 0.0])), Diagonal(3*SVector{model.ni[1],T}([1.0, 0.0, 1.0, 0.0]))] # Quadratic state cost
R = [Diagonal(0.3*SVector{model.mi[1],T}([1.0, 0.0])), Diagonal(3*SVector{model.mi[1],T}([1.0, 0.0]))] # Quadratic control cost
# Desrired state
xf = [SVector{model.ni[1],T}([1.0,+0.35,0.5,0]),
      SVector{model.ni[2],T}([1.3,-0.05,0.5,0]),
      ]
# Desired control
...
function get_figure(prob::Algames.GameProblem, sce::Scenario, vis_sol::Visualizer, vis_init::Visualizer)
    ...
    for k = 1:20
        ...
        if opt < opts.ϵ_opt && sta < opts.ϵ_sta && dyn < opts.ϵ_dyn && con < opts.ϵ_con
            ...
            plot_traj2!(model, init_traj.pr, plt=plt_init)
            plot_traj2!(model, sol_traj.pr, plt=plt_sol)
            push!(sol, sol_traj)
            push!(init, init_traj)
        end
    end
...
# Check singularity of the KKT system at convergence
# Dimension of the nullspace without regularization
residual_jacobian!(prob)
null = nullspace(Matrix(prob.core.jac))
dim = size(null)[2]

# Dimension of the nullspace with regularization
residual_jacobian!(prob)
regularize_residual_jacobian!(prob)
reg_null = nullspace(Matrix(prob.core.jac))
reg_dim = size(reg_null)[2]
